...
- Only Chromium browsers can run the models: Safari (WebKit) and Firefox (wgpu) cap maxStorageBuffersPerShaderStage at the spec default of 8, and the MLC kernels need 10 (baked into the compiled WASM; raising past 8 is a Dawn/Chromium capability). The start screen preflights the adapter limit and says so before any download; the runtime error is also translated into plain language.
